...
if __name__ == "__main__":
    ...
    def get_planting_form(email_token):
        form_response = requests.get(
            central_url
            + "/v1/projects/1/forms/build_Tree-planting_1625058144/submissions.csv",
            headers={"Authorization": "Bearer " + email_token},
            stream=True,
        )
    ...
    def clean_planting_csv():
        ...
        DIGITS = 16
        data_raw["asset_id"] = [
            int(uuid.uuid4().hex[:DIGITS], base=16) for _ in range(len(data_raw.index))
        ]
        ...
        data_raw[["date_of_planting", "time_of_planting"]] = data_raw["Time"].str.split(
            "T", 1, expand=True
        )

        # reformat date to mm/dd/yyyy
        data_raw["date_of_planting"] = pd.to_datetime(
            data_raw["date_of_planting"]
        ).dt.strftime("%m/%d/%Y")
        ...
        #deal with second DeviceID column that Pandas can't delete on first pass
        data_raw = data_raw.drop(
            columns=[
                "DeviceID.1"
            ]
        ...
        data_raw["Photo_planted_sapling"] = (
            "https://senecaparkzoo.getodk.cloud/v1/projects/1/forms/build_Tree-planting_1625058144/submissions/"
            + data_raw["ODK_meta-instanceID"].astype(str)
            + "/attachments/"
            + data_raw["Photo_planted_sapling"].astype(str)
        )
        ...
        if set(["asset_id", "latitude", "longitude"]).issubset(data_raw.columns):
            ...
            raise Exception(
                "csv needs asset_id, latitude, and longitude columns in order to process"
            )
